...
void PVV::ChunkUpdater::DecodeAll(bool mask_update_needed)
{
	...
	{
		uint32_t offset = rle_decode(active_chunk->compressed_data, active_chunk->compressed_len, decompressed_cache);
...
uint32_t PVV::ChunkUpdater::Compile(uint32_t* inds_p, glm::ivec3& min_bounds, glm::ivec3& max_bounds)
{
	...
	max_bounds.x >>= 11;
	max_bounds.z >>= 6;
	max_bounds.x--; max_bounds.z--;

	min_bounds.x >>= 11;
	min_bounds.z >>= 6;
	min_bounds.x--; min_bounds.z--;
...
bool PVV::ChunkUpdater::RayCast(const glm::vec3& o, const glm::vec3& dir, glm::vec3& intersection)
{
	...
	if (PVM::Intersections::Ray_AABB(o_rel, dir, glm::vec3(0), glm::vec3(Chunk::ChunkSide - 2, Chunk::ChunkLayers - 2, Chunk::ChunkSide - 2), t_near_surf, t_far_surf)) {
		//Compute next voxel along the path
		while (t_near_surf <= t_far_surf) {
			glm::vec3 surf_pos = (o_rel + dir * t_near_surf);

			if (surf_pos.x >= 0 && surf_pos.x < Chunk::ChunkSide - 2 && surf_pos.z >= 0 && surf_pos.z < Chunk::ChunkSide - 2 && surf_pos.y >= 0 && surf_pos.y < Chunk::ChunkLayers - 2) {
				uint64_t mask = 1ull << (uint64_t)(surf_pos.y + 1);
				if (mask_cache[Chunk::EncodeXZ((uint8_t)surf_pos.x, (uint8_t)surf_pos.z)] & mask) {
