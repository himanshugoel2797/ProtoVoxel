...
PPC::MeshMalloc::MeshMalloc() : alloc_lock()
{
}

PPC::MeshMalloc::~MeshMalloc()
{
	mem_blk.Unmap();
}

void PPC::MeshMalloc::Initialize()
{
...
uint32_t* PPC::MeshMalloc::Alloc(size_t sz, uint32_t& loopback_cnt_ret)
{
	alloc_lock.lock();
...
void PPC::MeshMalloc::Flush(uint32_t* offset, uint32_t len)
{
	FlushData flush_dat;
...
uint32_t PPC::MeshMalloc::GetOffset(uint32_t* ptr) {
	return ptr - mem_blk_ptr;
}

uint32_t PPC::MeshMalloc::GetCurrentLoopbackCount()
{
	return loopback_cnt;
}

ProtoVoxel::Graphics::GpuBuffer* PPC::MeshMalloc::GetBuffer()
{
	return &mem_blk;
}